...
public abstract class Shell {
	...
	new Command(this, "run", "run a class method") {
		...
		try {
		    if (args.length > 0)
			mdShell().run(args[0], Arrays.copyOfRange(args, 1, args.length));
		    else
	...
	new Command(this, "help", "this command help") {
	    void execute(String[] args) {
		if (args.length == 0) {
		    for (String s : mdShell().list())
			System.out.println(s);
		    System.out.println(name() + " name (for command details)");
		} else {
		    for (String s : args)
			if (mdShell().get(s) != null)
			    System.out.println(mdShell().get(s).help());
		}
	    }
	};
    }
	...
	if (prompt == null)
	    prompt = "$ ";
	System.out.println(name + " mdShell ('quit' to leave)");
    ...
    /**
     * Call a static method with argument String[] (Return value is ignored)
     * 
     * @param name
     *            full-class-name or full-method-name
     * @param args
     ...
     *            string to match
     * @param pattern
     *            string containing '*' (only) wildcards
	...
	for (String card : cards) {
	    ...
	    if (idx == -1)
		return false; // Card not detected in the text.
